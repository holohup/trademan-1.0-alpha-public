...
def get_open_and_close_time(func): # min = open time, max = close time
    hours_values = [t for tupl in OFFSET_ADJUSTED_WORK_HOURS.values() for t in tupl]
    return func(hours_values)


def perform_working_hours_check() -> bool:
    current_time = datetime.now(ZONE)
    if current_time.weekday() < WORK_DAYS[0] or current_time.weekday() > WORK_DAYS[1]:
        return False
...
def seconds_till_open_from_midnight() -> int:
    ...
    seconds_till_open = open_time.hour * 60 * 60 + open_time.minute * 60 + open_time.second
...
def get_seconds_till_open() -> int:
    ...
    sess_start_times = [period[0] for period in OFFSET_ADJUSTED_WORK_HOURS.values()]
